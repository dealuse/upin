...
<h3><span id="order-pay-dialog-close" class="close" onclick="return X.boxClose();">关闭</span><?php echo $INI['system']['couponname']; ?>验证及消费登记</h3>
	...
	<p class="act"><input id="coupon-dialog-query" class="formbutton" value="查询" name="query" type="submit" onclick="return X.coupononly.dialogquery();"/>&nbsp;&nbsp;&nbsp;<input id="coupon-dialog-consume" name="consume" class="formbutton" value="消费" type="submit" onclick="X.onlycoupon.couponconsume();" ask="每张<?php echo $INI['system']['couponname']; ?>只能消费一次，确定消费吗？"/></p>
</div>

<script type="text/javascript">

X.coupononly = {};
X.coupononly.dialogquery = function() {
	var id = jQuery('#coupon-dialog-input-id').val();
